...
function renderDashboard() {
  ...
  const topAliadosComb = k.porAliado.slice(0, 8);
  ...
  if (document.getElementById('chartGaugeScore')) {
    const todosLosScores = actas.map(a => parseFloat(a['Score'])).filter(v => !isNaN(v));
  ...
  if (document.getElementById('chartRadarAliados')) {
    ...
    const topPorVolumen = k.porAliado.slice().sort((a, b) => b.actas - a.actas).slice(0, 4);
    const maxActas = Math.max(...k.porAliado.map(a => a.actas), 1);
...
function configurarVistaProyectos() {
  document.getElementById('btnNuevoProyecto').addEventListener('click', abrirModalNuevoProyecto);
  document.getElementById('btnCancelarNuevoProyecto').addEventListener('click', () => cerrarModal('modalNuevoProyecto'));
  document.getElementById('formNuevoProyecto').addEventListener('submit', onCrearProyecto);
  document.getElementById('btnVolverProyectos').addEventListener('click', mostrarListaProyectos);
  document.getElementById('btnEliminarProyecto').addEventListener('click', onEliminarProyectoActual);
  document.getElementById('btnPresentarProyecto').addEventListener('click', abrirPresentacionProyecto);
  document.getElementById('btnExportarProyecto').addEventListener('click', exportarProyectoActual);
  document.getElementById('btnLimpiarProyecto').addEventListener('click', onLimpiarProyectoActual);
  document.getElementById('inputReemplazarExcel').addEventListener('change', onReemplazarExcelProyecto);
  document.querySelector('[data-view="proyectos"]').addEventListener('click', mostrarListaProyectos);
...
function configurarAsistenteProyectos() {
  ...
  const cerrar = () => { panel.classList.remove('is-active'); overlay.classList.remove('is-active'); };
  document.getElementById('btnCerrarAsistenteProyectos').addEventListener('click', cerrar);
  overlay.addEventListener('click', cerrar);
  document.getElementById('formPreguntaAsistenteProyectos').addEventListener('submit', (e) => {
    e.preventDefault();
